...
def getTrends():
    ...
    for location in loc:
        if location.loc == None:
            ...
            try:
                for trend in t.trends_place(location.woeid):
                    for n in trend['locations']:
                        location.name = n['name']
                        db.session.commit()
            except:
                location.name = 'ERROR no trends available for WOEID {}'.format(location.woeid)
                db.session.commit()

            for tag in (trend['trends']):

                checkTrend = models.TWITTER_TRENDS.query.filter(models.TWITTER_TRENDS.name == tag['name']).filter(
                    models.TWITTER_TRENDS.trend_loc == location.row_id).first()
